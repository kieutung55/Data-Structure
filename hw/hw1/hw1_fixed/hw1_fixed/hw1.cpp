...
public:
    ...
    // Remove a credential node at 'pos' position  from the linked list
...
void LinkedList::sort(string attribute) {
    for (int i = 0; i < this->size - 1; i++) {
        for (int j = this->size - 1; i < j; j--) {
            ...
            Node* node_j = this->getAtPos(j);
            if (attribute == "name" && node_i->name > node_j->name ||
                attribute == "age" && node_i->age < node_j->age ||
                attribute == "deposit" && node_i->deposit > node_j->deposit ||
                attribute == "number of drinks" && node_i->numDrinks < node_j->numDrinks) {
                this->swap(node_i, node_j);
            }
        }            
    }        
}
